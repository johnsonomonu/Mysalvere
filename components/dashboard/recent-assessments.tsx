    ...
    <div className="rounded-2xl border border-[#E7E5E4] bg-white p-6 shadow-sm">
      ...
      {assessments.length === 0 ? (
        <div className="rounded-lg border border-[#E7E5E4] bg-[#FAFAF9] p-4 text-sm text-[#57534E]">
          No assessments yet. Take your first one to unlock personalized tracking.
        </div>
      ) : (
        <div className="space-y-3">
          {assessments.map((assessment) => (
            <div
              key={assessment.id}
              className="flex items-center gap-3 p-3 rounded-lg border border-[#E7E5E4] bg-[#FAFAF9] hover:border-[#D6D3D1] transition-colors"
            >
              <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-[#1C1917]">
                <FileText className="h-5 w-5 text-[#F5F5F4]" />
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-[#1C1917]">{assessment.date}</p>
                <p className="text-xs text-[#57534E]">Priority: {assessment.priorityArea}</p>
              </div>
